...
def is_win(game_matrix, current_player_figure):
    ...
    for row in game_matrix:
        if not figure_won:
            figure_won = (row[0] == current_player_figure and row[1] == current_player_figure and row[
                2] == current_player_figure)
        else:
            break
    """
    ...
    for i in range(0, 3):
        if not figure_won_cols:
            figure_won_cols = len(list(filter(lambda x: x[i] == current_player_figure ,game_matrix))) == 3
                #(game_matrix[i][0] == current_player_figure and game_matrix[i][1] == current_player_figure and
                          #game_matrix[i][2] == current_player_figure)
        else:
            break
...
while True:
    ...
    while set_result != "OK":
        ...
        coords = input(set_result + " Введите веpные координаты ячейки (1-3<Разделитель>1-3):")
        set_result = set_figure(coords, game_matrix, current_player_figure)
